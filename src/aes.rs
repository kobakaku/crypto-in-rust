...
pub fn encrypt(input: [u8; 16], rounds: u8) -> [u8; 16] {
...
fn add_round_key(state: &mut [[u8; 4]; 4]) {
    todo!()
}

fn substitute(byte: u8, encryption: bool) -> u8 {
    let upper_nibble = ((byte >> 4) & 0b1111) as usize;
    let lower_nibble = (byte & 0b1111) as usize;
    if encryption {
        SBOX[upper_nibble][lower_nibble]
    } else {
        INVERSE_SBOX[upper_nibble][lower_nibble]
    }
}
